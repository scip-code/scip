...
}:
{
  ...
  go-bindings = pkgs.buildGoModule {
    ...
    vendorHash = "sha256-4c/UzeaEGyJZVsSJolGm43eQD8rjigrveqG6RZ0u6Ss=";
    env.GOWORK = "off";
    buildTags = [ "asserts" ];
    subPackages = [
      "."
  ...
  reprolang =
    ...
    pkgs.buildGoModule {
      ...
      src = ./.;
      modRoot = "./reprolang";
      vendorHash = "sha256-Y6NTn22Tk0JY1Rei/SqQ1cXav3g7MS/IF7+sXczgLf4=";
      proxyVendor = true;
      env.GOWORK = "off";
  ...
  typescript-bindings =
    ...
    pkgs.buildNpmPackage {
      ...
      npmDepsHash = "sha256-hLncSM2iXLVl3F489w3iSunHZ2+QG1jf16E47qRg+dk=";
      buildPhase = ''
        runHook preBuild
        npm run build
        runHook postBuild
